...
An array of objects associating dimensions to reference systems. Each object has a `"dimensions"` property and a `"system"` property. The `"dimensions"` property lists the identifiers of the dimensions for which the reference system applies, while the `"system"` property is a reference system object. For now, reference system objects are exactly as defined in the [CoverageJSON](https://github.com/Reading-eScience-Centre/coveragejson/blob/master/spec.md#5-reference-system-objects) format.
